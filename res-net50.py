...
def get_pretrained_resnet50(num_classes=10):
    model = models.resnet50(pretrained=True)
    model.fc = nn.Linear(model.fc.in_features, num_classes)
    return model

# Main training function
def train_pretrained_resnet_50():       
    # Load CIFAR-10 dataset
    training_data = datasets.CIFAR10(root='./data', train=True, transform=transform, download=True)
    train_loader = torch.utils.data.DataLoader(dataset=training_data, batch_size=batch_size, shuffle=True)
    
    # Initialize the pretrained model
    model = get_pretrained_resnet50(num_classes=num_classes).to(device)
...
def test_model():
    ...
    model = get_pretrained_resnet50(num_classes=10)
